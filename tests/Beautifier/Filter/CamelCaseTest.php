...
class CamelCaseTest extends PHPUnit_Framework_TestCase {
     ...
     */
    function testCamelCaseSample() {
        $sInput = dirname(__FILE__) . '/camel_input_file.phps';
        $sInputContent = file_get_contents($sInput);
        $sExpected = dirname(__FILE__) . '/camel_expected_file.phps';
        $sExpectedContent = file_get_contents($sExpected);
        $this->oBeaut->setInputFile($sInput);
        $this->oBeaut->addFilter("CamelCase");
        $this->oBeaut->process();
